# AkashicInstallPlan.ps1 — Generate an install plan for the Akashic integrity adapter
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
    [string]$PackageRoot,
...
if (-not (Test-Path $ManifestPath)) {
    throw "Package manifest not found: $ManifestPath. Run AkashicPackageValidation first."
}
...
$FilesToCopy = @(
    @{
        source = Join-Path $PackageRoot 'AkashicIntegrityBridge.ps1'
...
$RebaselinePlan = @{
    tool       = 'tools\AkashicEnvelopeManifest.ps1'
    parameters = @{
        HeliosGateRoot = $HeliosTargetRoot
        RebaselinedBy  = 'installer'
        Note           = 'Installation rebaseline'
    }
    verify_tool = 'tools\AkashicEnvelopeIntegrityValidation.ps1'
    verify_parameters = @{
        HeliosGateRoot = $HeliosTargetRoot
    }
...
$InstallPlan = [ordered]@{
    ...
    steps                 = @(
        @{ order = 1;  action = 'Verify package checksum'; tool = 'AkashicPackageValidation.ps1'; blocking = $true }
        ...
        @{ order = 6;  action = 'Generate local manifest'; tool = 'AkashicEnvelopeManifest.ps1'; blocking = $true }
        @{ order = 7;  action = 'Verify envelope integrity'; tool = 'AkashicEnvelopeIntegrityValidation.ps1'; blocking = $true }
